...
specInline = describe "inline" do
  ...
  describe "simple inlines" do
    serializeRoundtrip
      (runReaderT genSimpleInline genConfig)
      sI pI
 ...
 where
   ex txt rst = it ("should parse " ++ show txt) do
    case runSimpleParser (pInline <* eof) txt of
      Left err -> expectationFailure (errorBundlePretty err)
      Right d -> d `shouldBe` rst
...
myTripping gen s p = prop "can serialize and parse" do
  i <- forAll gen
  tripping i
    (runSimpleSerializer s)
    (runSimpleParser (p <* eof))
